...
export default function AdminDataModels() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      try { entities = JSON.parse(form.entities || '[]'); } catch { toast.error('Invalid entities JSON'); setSaving(false); return; }
      try { relationships = JSON.parse(form.relationships || '[]'); } catch { toast.error('Invalid relationships JSON'); setSaving(false); return; }
      const tags = form.tags.split(',').map(t => t.trim()).filter(Boolean);
      const body = { title: form.title, domain: form.domain, description: form.description, entities: JSON.stringify(entities), relationships: JSON.stringify(relationships), tags: JSON.stringify(tags) };

      if (modal === 'add') {
        await api.post('/data-models', body);
        toast.success('Model created');
      } else {
        await api.put(`/data-models/${modal.id}`, body);
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-white/5">
              ...
              <th className="text-left px-4 py-3 text-xs text-slate-500 uppercase tracking-wider">Entities</th>
          ...
          <tbody>
            {(models || []).map(m => (
              <tr key={m.id} className="border-b border-white/5 hover:bg-white/[0.02]">
                ...
                <td className="px-4 py-3 text-slate-400">{(m.entities || []).length}</td>
